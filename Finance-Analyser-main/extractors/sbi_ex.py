...
class SbiExtractor(BaseExtractor):
    ...
    def extract_metadata(self, text: str):
        ...
        if not period_match:
            period_match = re.search(r"Account\s*Statement\s*from\s*(\d{1,2}\s+[A-Za-z]{3}\s+\d{4})\s*to\s*(\d{1,2}\s+[A-Za-z]{3}\s+\d{4})", text, re.I)
        if period_match:
        ...
        if name_match:
            ...
            # Clean up trailing label markers if any
