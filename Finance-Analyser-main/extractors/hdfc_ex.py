    ...
    def extract_metadata(self, text: str):
        ...
        if name_match:
            ...
            for idx, line in enumerate(lines):
                if "ADDRESS" in line and not "BRANCH ADDRESS" in line:
                    if idx > 0:
                        # Collect preceding lines up to account number or start
                        name_parts = []
                        for k in range(max(0, idx - 3), idx):
                            l = lines[k]
                            if re.match(r"^\d+$", l) or any(w in l.lower() for w in ["statement", "period", "currency", "branch"]):
                                continue
                            name_parts.append(l)
                        if name_parts:
                            res_name = " ".join(name_parts).strip()
                            res_name = res_name.split("BRANCH ADDRESS")[0].strip()
                            self.metadata["customer_name"] = res_name
                        else:
                            self.metadata["customer_name"] = lines[idx-1].split("BRANCH ADDRESS")[0].strip()
                        break
