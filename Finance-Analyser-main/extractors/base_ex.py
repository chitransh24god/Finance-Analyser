...
class BaseExtractor:
    ...
    def extract_text_pypdf(self, pdf_path: str, password: str = None) -> str:
        ...
        Uses pdfplumber primary for high-fidelity text extraction with pypdf fallback.
        """
        return self.extract_text_pdfplumber(pdf_path, password)

    def extract_text_pdfplumber(self, pdf_path: str, password: str = None) -> str:
        """
        Helper to extract raw text using pdfplumber, replacing CID control characters.
        """
        try:
            full_text = ""
            with pdfplumber.open(pdf_path, password=password) as pdf:
                for i in range(min(2, len(pdf.pages))):
                    ...
                    full_text += page_text + "\n"
            # Clean control/CID characters like (cid:9)
            full_text = re.sub(r"\(cid:\d+\)", " ", full_text)
            return full_text
        except Exception as e:
            log_warning(f"pdfplumber text extraction failed, trying pypdf: {e}")
            try:
