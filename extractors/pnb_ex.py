...
class PnbExtractor(BaseExtractor):
    def __init__(self):
        super().__init__("Punjab National Bank")
    ...
    def parse(self, file_path: str, password: str = None) -> tuple[dict, list[dict]]:
        log_info(f"Parsing PNB statement: {os.path.basename(file_path)}")
        ...
        with pdfplumber.open(pdf_path, password=password) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    if ("txndate" in headers or "date" in headers or "valuedate" in headers) and "balance" in headers:
                        date_idx = headers.index("txndate") if "txndate" in headers else (headers.index("date") if "date" in headers else 0)
                        ...
                        deb_idx = headers.index("withdrawal(dr)") if "withdrawal(dr)" in headers else (headers.index("debit") if "debit" in headers else 3)
                        cred_idx = headers.index("deposit(cr)") if "deposit(cr)" in headers else (headers.index("credit") if "credit" in headers else 4)
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"PNB extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
