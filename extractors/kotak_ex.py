...
class KotakExtractor(BaseExtractor):
    def __init__(self):
        super().__init__("Kotak Mahindra Bank")
    ...
    def extract_metadata(self, text: str):
        ...
        # Customer Name
        name_match = re.search(r"Customer\s*Name\s*:?\s*([A-Za-z0-9\s\.\&]+?)(?:\n|Account|CRN|$)", text, re.I)
        if name_match:
            self.metadata["customer_name"] = name_match.group(1).strip()

    def parse(self, file_path: str, password: str = None) -> tuple[dict, list[dict]]:
        log_info(f"Parsing Kotak statement: {os.path.basename(file_path)}")
        ...
        with pdfplumber.open(pdf_path, password=password) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    if ("date" in headers or "slno" in headers or "txndate" in headers) and "balance" in headers:
                        date_idx = headers.index("date") if "date" in headers else (headers.index("txndate") if "txndate" in headers else 1)
                        ...
                        deb_idx = headers.index("withdrawal") if "withdrawal" in headers else (headers.index("debit") if "debit" in headers else 3)
                        cred_idx = headers.index("deposit") if "deposit" in headers else (headers.index("credit") if "credit" in headers else 4)
                        bal_idx = headers.index("balance") if "balance" in headers else -1
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"Kotak extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
