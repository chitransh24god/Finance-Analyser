...
class BobExtractor(BaseExtractor):
    def __init__(self):
        super().__init__("Bank of Baroda")
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    if ("date" in headers or "valuedate" in headers) and "balance" in headers:
                        ...
                        desc_idx = headers.index("narration") if "narration" in headers else (headers.index("description") if "description" in headers else 1)
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"Bank of Baroda extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
