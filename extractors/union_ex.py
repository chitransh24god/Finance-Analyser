...
class UnionExtractor(BaseExtractor):
    def __init__(self):
        super().__init__("Union Bank of India")
    ...
    def parse(self, file_path: str, password: str = None) -> tuple[dict, list[dict]]:
        log_info(f"Parsing Union Bank statement: {os.path.basename(file_path)}")
        ...
        with pdfplumber.open(pdf_path, password=password) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    if ("date" in headers or "valuedate" in headers) and "balance" in headers:
                        date_idx = headers.index("date") if "date" in headers else 0
                        desc_idx = headers.index("remarks") if "remarks" in headers else (headers.index("narration") if "narration" in headers else 1)
                        deb_idx = headers.index("debit") if "debit" in headers else 2
                        cred_idx = headers.index("credit") if "credit" in headers else 3
        ...
        for idx, row in enumerate(rows):
            ...
            if any("date" in c for c in row_clean) and any("balance" in c for c in row_clean):
                ...
                for c_idx, c_val in enumerate(row_clean):
                    if "date" in c_val:
                        col_map["date"] = c_idx
                    elif "particulars" in c_val or "remarks" in c_val or "narration" in c_val:
                        col_map["desc"] = c_idx
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"Union Bank extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
