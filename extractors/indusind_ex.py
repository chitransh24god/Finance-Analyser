...
class IndusindExtractor(BaseExtractor):
    def __init__(self):
        super().__init__("IndusInd Bank")
    ...
    def parse(self, file_path: str, password: str = None) -> tuple[dict, list[dict]]:
        log_info(f"Parsing IndusInd Bank statement: {os.path.basename(file_path)}")
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"IndusInd Bank extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
