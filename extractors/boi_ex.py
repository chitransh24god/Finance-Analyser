...
class BoiExtractor(BaseExtractor):
    ...
    def parse(self, file_path: str, password: str = None) -> tuple[dict, list[dict]]:
        log_info(f"Parsing Bank of India statement: {os.path.basename(file_path)}")
    ...
    def _finalize_transactions(self, transactions: list[dict]) -> list[dict]:
        ...
        log_info(f"Bank of India extraction complete. Rows: {len(cleaned_txs)}")
        return cleaned_txs
