...
class AxisExtractor(BaseExtractor):
    ...
    def extract_metadata(self, text: str):
        ...
        # Date range
        period_match = re.search(r"period\s*\(From\s*:\s*(\d{2}-\d{2}-\d{4})\s*To\s*:\s*(\d{2}-\d{2}-\d{4})\)", text, re.I)
        if not period_match:
        ...
        # Customer Name
        # Search lines before "Joint Holder" or similar
        ...
        if self.metadata["customer_name"] == "Not Available":
            name_match = re.search(r"^([A-Z\s]{4,35})$", lines[0] if lines else "")
            if name_match:
